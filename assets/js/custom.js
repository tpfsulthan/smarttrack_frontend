...
//  $(document).on("click",".bayId",function() {
  ...
  function initMap() {
    ...
    var myOptions = { zoom: 14, disableDefaultUI:true,center: new google.maps.LatLng(12.949152, 80.185317),styles : style,center: new google.maps.LatLng(12.9490, 80.1828), mapTypeId: google.maps.MapTypeId.ROADMAP };
...
function calcRoute(start, end, FromRes,icon,item) {
  ...
  var directionsDisplay = new google.maps.DirectionsRenderer({
      draggable: false,
      map: map,
      polylineOptions: { strokeColor: RouteColor[item], strokeOpacity: 0.4, strokeWeight: 5 },
      markerOptions: { title: FromRes,icon:icon }     
  });
...
function ShowPolyLine(result, Title) {
  ...
  var Filter = $(Routes).filter(function () { return this.Title == Title; })
  var lineSymbol = { scale: 3, fillColor: '#FF0000',fillOpacity: 0.7, strokeOpacity: 0.7, strokeWeight: 4 };
  Filter[0].PolyLine = new google.maps.Polyline({ map: map, path: RouteCoordinates, icons: [{ icon: lineSymbol, offset: '100%' }], geodesic: true, strokeColor: "#0094ff",strokeOpacity: 1.0,strokeWeight: 5 });
}
